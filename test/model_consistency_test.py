...
def test_predictions():
    ...
    with graph.as_default():
        ...
        for cur_word in test_sentence.split():
            ...
            predicted_words = sess.run( graph_nodes['predicted_words'],
                                                feed_dict={input_seqs: cur_encoded_word[0]}).tolist()

            cell_states = sess.run( graph_nodes['lstm_state'],
                                                feed_dict={input_seqs: cur_encoded_word[0]})

            print(cell_states.shape)

            actual_predictions.extend([ [p.decode("utf-8")  for p in pp]for pp in predicted_words])


        print(list(zip(expected_predictions,actual_predictions)))
