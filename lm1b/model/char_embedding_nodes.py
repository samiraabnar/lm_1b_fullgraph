...
def _create_cnn_cell( inputs, kernel_size, kernel_feature_size, name="cnn", hparams=None ):
    ...
    with tf.variable_scope( name ):
        ...
        out = tf.nn.relu( conv )
        out = tf.reduce_max( out, reduction_indices=[1], keepdims=True )
...
def _highway_layer( input, num_shards= 8, name="dense"):
    ...
    with tf.variable_scope(name):
        with tf.variable_scope( "gate" ):
            t_gate = tf.nn.sigmoid( sharded_linear( input, (int(input.shape[1]) / num_shards, input.shape[1]), num_shards=int(num_shards) ) - 2.0 )

        with tf.variable_scope( "tr" ):
            tr_gate = tf.nn.relu( sharded_linear( input, (int(input.shape[1]) / num_shards, input.shape[1]), num_shards=int(num_shards) ) )
